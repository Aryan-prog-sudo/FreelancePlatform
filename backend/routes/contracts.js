...
router.post('/accept-bid', async (req, res) => {
  ...
  try {
    const contract_id = Math.floor(Math.random() * 900000) + 100000;
    await db.query(
      `INSERT INTO CONTRACT VALUES (?, ?, ?, 'Signed')`,
      [contract_id, project_id, freelancer_id]
    );
    res.json({ message: 'Bid accepted! Contract created and escrow funded automatically.', contract_id });
  } catch (err) {
    console.error(err);
